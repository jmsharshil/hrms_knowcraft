...
from .serializers import CandidateSerializer, BookingSerializer
...
class SendSlotSelectionEmailView(APIView):
    def post(self, request):
        ...
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [candidate.email], fail_silently=False)
...
class CandidateBookSlotView(APIView):
    ...
    def post(self, request, candidate_id):
        ...
        event = create_teams_meeting(
            ...
            start_dt,
            end_dt,
            subject=f"Interview: {candidate.full_name}"
        )
        ...
        send_mail(
            ...
            f"Hello {interviewer.name},\nYou have an interview with {candidate.full_name} at {start_str}.\nJoin link: {meeting_link}",
            settings.DEFAULT_FROM_EMAIL,
            [interviewer.email],
        )
